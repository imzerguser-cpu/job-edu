import {collection,doc,getDocFromServer,getDocsFromServer,limit,query,runTransaction,serverTimestamp,setDoc,Timestamp,type Firestore} from 'firebase/firestore';
import {isTeacher,schoolPath,type SchoolContext} from '../domain/model';
import {assertApplicant} from '../domain/jobs';
import {
  canClose,entityIdOf,validateBusinessFields,validateJobFields,
...
export interface ProposalStore {
  ...
  submit(type:ProposalType,fields:JobProposalFields|BusinessProposalFields):Promise<void>;
...
export function firestoreProposals(db:Firestore,context:SchoolContext):ProposalStore{
  ...
  return {
    ...
    async submit(type,fields){
      ...
      else validateBusinessFields(fields as BusinessProposalFields);
    ...
    async decide(proposal,approve,note,extra){
      ...
      const entityId=approve?entityIdOf(proposal.id):null;
      await runTransaction(db,async tx=>{
        const target=ref('proposals',proposal.id),old=await tx.get(target);
