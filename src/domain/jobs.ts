...
export interface Career {id:string;schoolId:string;departmentId:string;name:string;description:string;core:boolean;status:JobState;recommendedGrades:number[];icon:string;schemaVersion:1}
...
export function starterJobs(schoolId:string):Career[]{
  const items=[
    ['bank','economy','은행원','거래 내역을 확인하고 친구들의 금융생활을 도와요.','banker',true,[5,6]],
    ['reporter','media','기자','학교의 소식을 취재하고 기사를 써요.','journalist',false,[3,4,5,6]],
    ['garden','life','식물관리원','화분을 살피고 식물이 잘 자라도록 돌봐요.','plant_manager',true,[1,2,3]],
    ['library','life','도서관 관리원','책을 정리하고 책을 빌리는 친구들을 도와요.','library_manager',true,[1,2,3,4]],
    ['events','culture','행사기획자','친구들의 의견을 모아 학교 행사를 준비해요.','event_planner',false,[3,4,5,6]],
    ['store','economy','상점 관리원','물건을 정리하고 판매 활동을 도와요.','store_manager',false,[3,4,5,6]],
  ] as const;
  return items.map(([id,departmentId,name,description,icon,core,grades])=>({id,schoolId,departmentId,name,description,icon,core,recommendedGrades:[...grades],status:'recruiting',schemaVersion:1}));
}
