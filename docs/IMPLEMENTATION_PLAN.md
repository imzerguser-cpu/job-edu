...
## 27. 공동기금 지출, 그리고 Rules 평가 한도를 실제로 넘겼던 사건 (신규)

2026-09-14: 소득세·사업 세금·과태료로 걷은 돈이 쌓이기만 하고 쓸 곳이 없던 공동기금(§23)에 교사가 직접 지출할 수 있는 기능을 추가했다. 구매(D-24)처럼 미리보기 없이 즉시 실행되는 한 번뿐인 사건이고, 지출한 돈은 실제로 받는 쪽 계좌가 없으므로(실세계로 나가는 돈) 발행 계좌(`system-issuer`)로 돌려보내 전체 계좌 합=0(D-19) 불변식을 지킨다.

이 기능을 붙이자마자 **소득세·사업세·과태료를 포함해 이미 배포된 모든 저널 생성이 위태로워질 뻔한** 구조적 문제를 발견해 고쳤다: `journals`의 `allow create`가 저널 타입 9개를 OR로 묶은 거대한 조건식으로 자라 있었는데, 그중 여러 분기가 매번 `teacher(sid)`/`activeStudent(sid)`/`callerStudentId(sid)`를 다시 계산(각각 자체 `get()` 호출 포함)하면서 한 요청의 누적 평가 비용이 Firestore의 요청당 표현식 평가 한도(약 1000개)를 실제로 넘겨, 조건이 명백히 참인데도 전체가 `PERMISSION_DENIED`로 거부되는 걸 에뮬레이터에서 재현했다. `journalCreateOk(journalId)` 함수로 묶고 `let`으로 한 번만 계산해 재사용하도록 고쳤다 — 각 분기가 검사하는 조건은 전혀 바꾸지 않아 기존 143개 테스트가 무수정으로 통과했다. 자세한 근거와 재사용 가능한 교훈은 `docs/DECISIONS.md` D-73.

## 28. 경제 통계 1차 구현 (신규)

2026-09-14: §23의 마지막 항목인 경제 통계를 운영 현황 화면에 추가했다. "유통 중인 마동"(학생·사업 계좌 잔액의 합, 발행·공동기금 계좌는 제도 자체이므로 제외), 학생 평균 잔액, 사업 계좌 합계, 공동기금 잔액을 보여준다. 이미 존재하는 `accounts.list`(교사 전용, 상한 100) 조회를 그대로 재사용해 클라이언트에서 집계하므로 새 Rules나 색인이 전혀 필요 없었다 — 이번 단계 중 유일하게 `firebase/firestore.rules`를 건드리지 않은 기능이다. 근거는 `docs/DECISIONS.md` D-74.

이로써 §23(세무·회계)이 요구한 다섯 항목(소득세·사업 세금·공동기금·재정 장부·경제 통계) 모두, §24(과태료)까지 전부 최소 1차 구현이 연결됐다. "재정 장부"는 별도 화면 없이 각 계좌의 `entries`(불변 원장)로 이미 충족된다고 판단했다.

## 29. 시민광장 확장 — 규칙/행사/학교개선 제안, 초안, 댓글 (신규)

2026-09-14: §26이 예고했던 "새로운 직업, 사업, 서비스, 규칙, 학교 개선 아이디어, 행사"의 나머지(규칙·행사·학교 개선)를 연결하고, README가 "아직 없다"고 적어 뒀던 초안 저장과 시민 의견(댓글)도 함께 구현했다.

새 세 유형(rule/event/community)은 job/business와 달리 승인해도 새 시스템 문서를 만들지 않는다 — "학급이 이 아이디어를 채택한다"는 결정 자체가 결과물이라, 세 유형이 하나의 `SimpleProposalFields{title,description,reason}`을 공유한다(D-75). 초안은 새 컬렉션 없이 `proposals`에 `status:'draft'`를 추가해 구현했고, 가시성은 기존 제안과 동일(학급 전체 공개)으로 단순화했다 — 작성자만 수정·제출할 수 있도록 편집 권한만 엄격히 지킨다(D-76). 댓글(`proposals/{id}/comments`)은 journals/auditLogs와 같은 불변 원칙을 따른다(D-77).

이 단계에서도 D-73과 같은 패턴의 Rules 평가 한도 초과를 겪었다 — `proposals`의 `allow update`에 초안 분기 2개를 추가하자 기존 job/business 승인 테스트가 깨졌고, `journalCreateOk`와 같은 `let` 메모이제이션으로 즉시 고쳤다(D-78). 이제 "OR 분기가 많은 규칙에 새 분기를 추가하면 곧바로 전체 테스트를 돌려본다"가 이 코드베이스의 확립된 관행이 됐다.

## 30. 사진 인증 이력 1차 구현 (신규)

2026-09-14: 원본 계획 8단계까지, 그리고 이후 이어 붙인 모든 후속 단계(소득세~시민광장 확장)의 마지막 남은 항목을 연결했다. 사진 원본을 더 오래 보관하는 대신(D-37의 "개인정보 최소 보관" 원칙 유지), 심사 트랜잭션에서 증빙을 지우는 바로 그 자리에 감사 로그 한 줄(`action:'photo_review'`, 학생·시도 번호·승인 또는 다시 제출 요청)을 추가했다 — 새 컬렉션도 새 Rules도 새 화면도 필요 없었다(이미 있는 `auditLogs`와 운영 현황의 "최근 주요 활동" 피드를 그대로 재사용, D-79). 이 단계 전체 중 유일하게 `firebase/firestore.rules`를 전혀 건드리지 않은 기능이다.

여기까지로 2026-09-13~14 세션에서 진행한 모든 후속 구현(§21~30, 원본 계획 8단계 이후)이 일단락됐다 — 소득세·사업 세금·공동기금·경제 통계·과태료(§23~24), 사업 운영 학생 자기 관리(§25 후속), 시민광장 확장(§26 후속), 사진 인증 이력(§18 후속), 그리고 학생 비밀번호 셀프서비스(교사용 Cloudflare Worker) 및 학교 코드 정규화까지. 남은 것은 §22의 대출 분할·중도상환, 사업 세금 이후의 재정 장부 세부 통계, 그리고 2교사 실제 학교 데이터로의 배포·운영이다.

## 31. 구글 시트로 학생 비밀번호 일괄 변경 (신규)

2026-09-14: 교사가 한 명씩 "비밀번호 재설정"을 누르는 대신, 구글 시트 링크 하나로 최대 20명을 한 번에 바꿀 수 있게 했다. 시트 형식은 `scripts/admin-bootstrap.mjs`가 이미 출력하는 `student-accounts.csv`와 같은 열 순서(`이름,학년,학교코드,비밀번호`)라, 그 파일을 시트에 붙여넣고 몇 개만 고쳐서 공유 링크만 붙여넣으면 된다.

시트를 읽는 건 브라우저가 아니라 기존 Cloudflare Worker(D-56~D-58)다 — Google의 CSV 내보내기 엔드포인트가 제3자 origin에 CORS를 열어주지 않아 브라우저에서 직접 `fetch()`할 수 없기 때문이다(D-80). 새 엔드포인트·새 비밀키 없이 기존 비밀번호 재설정 요청에 `sheetUrl` 필드가 있으면 일괄 모드로 분기하도록만 확장했다. 실제 학생 계정 2명으로 `data:` URI를 이용한 엔드투엔드 테스트(계정 조회 → 비밀번호 변경 → 새 비밀번호로 로그인 성공까지)를 마치고 원래 비밀번호로 되돌려 뒀다. 근거는 `docs/DECISIONS.md` D-80~D-82.
