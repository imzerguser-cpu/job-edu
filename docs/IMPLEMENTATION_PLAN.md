...
## 17. 시민광장(제안·투표) 1차 구현 범위 (신규)

2026-09-13: `proposals`(직업/사업 제안, 구조화된 필드) + `proposals/{id}/votes/{uid}`(1인 1표)를 추가해 원문 §26-29의 핵심 순환(제안 → 투표 → 교사 승인 → 새 직업/사업 생성)을 실제로 연결했다. 학생 제출 → 교사가 마감·최소 참여율·찬성 기준을 정해 투표 시작 → 학생 찬반 투표(비공개, 1인 1표) → 마감 후 교사가 집계 → 승인 시 `jobs`/`businesses`(+계좌) 문서를 `proposal-{proposalId}` ID로 같은 트랜잭션에서 생성. 지도의 "마동초등학교(마이페이지)" 건물에 직업 신청·업무·계좌에 이어 네 번째로 연결됐다.

RULE/EVENT/COMMUNITY 제안 종류, 초안(draft) 저장, 시민 의견(댓글)은 아직 없다. 투표 집계는 Rules가 서브컬렉션을 세지 못하는 한계 때문에 교사 클라이언트가 계산해 제출한 값을 신뢰한다(개별 투표 자체는 완전히 보호됨). 자세한 근거와 새로 발견한 Rules 문법 함정(경로 리터럴에 문자열+변수를 함께 못 씀, 서브컬렉션 전수 조회는 별도 상한 필요)은 `docs/DECISIONS.md` D-27~D-32.

## 18. 사진 증빙(사진 인증) 1차 구현 범위 (신규)

2026-09-13: 업무(Task)의 `verificationKind='photo'`를 실제로 연결했다(3단계에서 "다음 단계"로 미뤄뒀던 부분, D-15). 브라우저에서 사진을 640px·60KB 이하 JPEG로 재인코딩(EXIF 자동 제거) → `evidence/{taskId}`에 Base64로 임시 저장(24시간 만료) → 교사 검토(승인/반려 어느 쪽이든) 시 같은 트랜잭션에서 원본 삭제. 만료된 사진은 규칙이 즉시 조회를 막고, 물리 삭제는 교사 접속 시 최대 20건씩 정리한다(Spark에 예약 작업이 없어 원문 §33 제약을 그대로 따름).

이 단계에서도 Rules 버그 두 개를 에뮬레이터로 잡았다: 복합 ID(`~` 포함)에 단순 ID 정규식을 그대로 복사해 쓴 실수, 그리고 삭제된 문서를 조회할 때 D-20과 같은 유형의 null-resource 오류(이번엔 계좌처럼 경로만으로 소유권을 판정할 수 없어 호출부에서 흡수). 근거는 `docs/DECISIONS.md` D-33~D-37.

## 19. 운영(통계·감사·학적 관리) 1차 구현 범위 (신규)

2026-09-13: 8단계(마지막 계획 단계)를 구현했다. 추가한 것: (1) `auditLogs` — 제안 승인/반려, 사업 생성, 월급 지급 3곳에 한해 같은 트랜잭션에서 "누가·언제·무엇을" 기록(원문 §62의 일부만 우선 구현, D-38), (2) 교사 명단 화면에 학생 학년·반·학적 상태(졸업/전출) 수정 폼 — 서버 규칙은 1단계부터 이미 이를 허용하고 있었고 화면만 없었다(D-41), (3) 운영 현황 대시보드 — 새 집계 쿼리 없이 기존 각 기능의 목록을 재사용해 개수만 센다(D-42). 접근성은 아이콘 전용 버튼·alt 누락 이미지가 있는지 가볍게 점검했고 별다른 문제를 찾지 못했다(전면 스크린리더 감사는 아님).

원문 §11 8단계의 통과 기준("2학교 실데이터 격리, 계정 비활성화, 비용 점검, 전체 사용자 흐름")은 1~7단계에서 누적된 86개 에뮬레이터 테스트가 이미 충족하고 있어 새 테스트를 추가하지 않았다(D-43). 로그인 계정(membership) 발급·비활성화는 여전히 앱 밖 관리 도구 영역이다(D-5, 변경 없음).

## 20. 이메일 없는 학생 로그인, 관리자 계정 발급 도구 (신규)

2026-09-13: §4가 "학생 개인 이메일을 가정하지 않는다"고 명시했던 부분을 실제로 구현했다. Firebase Auth의 이메일/비밀번호 로그인은 문자열이 이메일 형식이기만 하면 되므로, `{학교코드}-{학년}-{이름}@students.jobedu.local` 형태의 합성 이메일을 만들어 쓴다(`src/domain/studentAuth.ts`). 학생은 이 사실을 몰라도 되며, 로그인 화면의 "학생" 탭에서 학교 코드·학년·이름·비밀번호만 입력한다(학년당 반이 하나뿐인 학교를 기준으로 반을 뺐다 — D-44).

계정 발급 자체는 앱이 아니라 `scripts/admin-bootstrap.mjs`(Admin SDK, 서비스 계정 키로 실행하는 오프라인 도구)가 한다 — 이것이 §4가 예고한 "별도의 검증된 관리 도구"다. 학교 생성, 교사 계정, 그리고 이미 가져온 명단 전체의 학생 계정을 한 번에 만들고 비밀번호를 CSV로 출력한다(재실행 안전). 근거는 `docs/DECISIONS.md` D-44~D-47.

이로써 원본 계획 8단계 전체(1~8단계)의 핵심 기능이 한 차례씩 실제로 연결됐다. 남은 것은 심화 기능(저축·대출·세금·과태료, 규칙/행사/커뮤니티 제안, 사업 운영 학생 자기 관리 화면, 제출 이력 등)과 다듬기(대규모 목록 페이지네이션, 학교별 국 이름 커스터마이징 등) — 전부 `docs/DECISIONS.md`에 이미 "다음 단계"로 명시되어 있다.

## 21. 저축 1차 구현 범위, 대출은 제외 (신규)

2026-09-14: §6이 요구한 심화 금융 기능 중 **저축만** 연결했다. 대출은 이번 범위에서 제외했다 — 원문 §20이 대출에 "은행원 검토 → 필요 시 교사 승인"을 명시적으로 요구하는 것은 목적·상환계획에 대한 사람의 판단이 필요해서이고, 저축 가입은 원금·기간이 상품 범위 안인지와 잔액이 충분한지만 확인하면 되어 이미 구현된 구매(§16, D-23)와 정확히 같은 신뢰 수준이다. `financialRequests`/`financeReviews`(§5 표, 학생 요청 → 은행원 검토 워크플로)는 이번에도 도입하지 않았다 — 정말로 그 워크플로가 필요한 것은 대출이며, 두 번째 구체 사례(대출) 없이 먼저 일반화하지 않는다는 D-17의 원칙을 그대로 따른다.
...
## 23. 소득세 1차 구현 범위 — 사업 세금·과태료는 제외 (신규)

2026-09-14: §23이 요구한 "세무·회계" 중 **소득세만** 연결했다. 사업 세금·공동기금 지출·경제 통계와 §24의 과태료는 이번 범위에서 제외했다 — 소득세는 이미 존재하는 SALARY 저널을 그대로 과세 대상으로 삼을 수 있어 새 워크플로가 필요 없지만, 사업 세금은 구매/매출 흐름에, 과태료는 "규칙 위반 보고"라는 아직 없는 워크플로에 얽혀 있어 각각 별도 단계가 필요하다.
...
## 25. 사업 세금 1차 구현, 그리고 Rules "없는 필드" 버그 수정 (신규)

2026-09-14: 소득세(§23)의 짝인 사업 세금을 연결해 §23의 두 세목(소득세·사업 세금)이 모두 구현됐다. 설계는 소득세와 대칭이다 — 사업 계좌 잔액이 아니라 그 정산월의 실제 매출(PURCHASE로 사업 계좌에 들어온 금액)에만 매기고, 세율은 교사가 설정하며, `community-fund`(소득세와 같은 계좌)로 징수한다. PURCHASE 저널에는 `period` 필드가 없어(이미 배포된 `buy()`를 건드리지 않으려 그대로 둠) 사업별 `accounts/{id}/entries`(D-65에서 자기 매출 열람용으로 이미 만든 것)를 재사용해 `postedAt` 기준으로 달력월을 판정한다 — 학교 전체 `journals`를 등호+정렬 복합 쿼리로 훑는 대신, 새 복합 색인 없이 안전하게 구현했다(D-67~D-68).

이 단계에서 **소득세에도 영향을 주는 버그**를 찾아 고쳤다: Firestore Rules에서 `d.field == null`은 그 필드가 "존재하며 값이 null"일 때만 안전하고, 필드가 아예 없는 문서에서는 평가 오류를 던진다(D-62가 썼던 패턴이 처음부터 이 문제를 안고 있었음). `d.get('field', null) == null`로 두 세율 필드 모두 고쳤다. 자세한 근거는 `docs/DECISIONS.md` D-66~D-69.

## 26. 과태료 1차 구현 (신규)

2026-09-14: §24(과태료)를 연결해 §23~24가 요구한 세무·회계 항목이 모두 구현됐다. 흐름은 원문 그대로다: 규칙 위반 보고(`violationReports`, 누구나 — 학생 자신도, 교사도) → 교사 확인 → 과태료 결정(금액 지정 또는 기각, 사유 필수) → 시스템이 `FINE` 저널 생성(소득세·사업세와 같은 `community-fund`로 귀속) → 학생 계좌 즉시 차감 → 자기 거래 내역에 반영. 신고 자체는 아무 금융 효과가 없어 학생이 마음대로 과태료를 매길 방법이 없다(D-70).

신고 원문(신고자·내용)은 신고자 본인과 교사만 볼 수 있고, 대상 학생은 결과(자기 계좌의 "과태료" 거래 내역)만 본다 — 또래 신고이므로 개인정보를 최소로 노출한다(D-71). 이 기능을 만들며 처음으로 "학생이 다른 학생을 지목"해야 하는 화면이 생겨, 그동안 교사 전용이던 `students` 컬렉션의 `list` 규칙을 활동 중인 멤버 전체로 넓혔다(상한 100은 유지) — 작은 학급에서 이름·학년은 이미 서로 아는 정보라 새로 노출되는 것은 없다고 판단했다(D-72). 근거는 `docs/DECISIONS.md` D-70~D-72.
